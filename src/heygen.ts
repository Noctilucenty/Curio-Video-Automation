// HeyGen client wrapper. HeyGen ONLY renders — script, tone, and captions are
// decided upstream by the OpenAI package. Missing key => MockRenderer so the
// pipeline completes offline. Provider responses are sanitized before storage
// (ids/urls/status only — never raw payloads that could carry account info).
      ...
      video_inputs: [
        {
          ...
          voice: { type: "text", input_text: req.pkg.script, voice_id: req.voiceId },
          background: { type: "color", value: "#0B0B0F" },
        },
      ],
      dimension: { width: 1080, height: 1920 },
      // Captions are burned by our own style system, not HeyGen's default look.
      caption: false,
      title: req.pkg.title,
    };
...
export class MockRenderer implements Renderer {
  ...
  private seq = 0;
  async createVideo(_req: RenderRequest): Promise<{ providerVideoId: string }> {
